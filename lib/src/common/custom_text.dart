...
class _SelectableTextContainerDelegate
  ...
  SelectedContent? getSelectedContent() {
    if (selectables.isEmpty || selectables[0].getSelectedContent() == null) {
      return null;
    }

    String selected = selectables[0].getSelectedContent()!.plainText;
    ...
    for (int i = 0; i < selected.length; i++) {
      ...
      for (int j = 0; j < semantics.length; j++) {
        if ((i + 1) < selected.length &&
            selected[i] + selected[i + 1] == semantics[j].text) {
          buffer.write('${semantics[j].semanticsLabel}');
          break;
        } else if (selected[i] == semantics[j].text) {
          buffer.write('${semantics[j].semanticsLabel}');
          break;
        }
      }
